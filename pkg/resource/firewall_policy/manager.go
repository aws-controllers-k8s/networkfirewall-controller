...
func (rm *resourceManager) IsSynced(ctx context.Context, res acktypes.AWSResource) (bool, error) {
	...
	if r.ko.Status.FirewallPolicyResponse == nil {
		return false, nil
	}
	if r.ko.Status.FirewallPolicyResponse.FirewallPolicyStatus == nil {
		return false, nil
	}
	firewallPolicyResponseCandidates := []string{"ACTIVE"}
	if !ackutil.InStrings(*r.ko.Status.FirewallPolicyResponse.FirewallPolicyStatus, firewallPolicyResponseCandidates) {
		return false, nil
	}
