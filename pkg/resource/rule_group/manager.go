...
func (rm *resourceManager) IsSynced(ctx context.Context, res acktypes.AWSResource) (bool, error) {
	...
	if r.ko.Status.RuleGroupResponse == nil {
		return false, nil
	}
	if r.ko.Status.RuleGroupResponse.RuleGroupStatus == nil {
		return false, nil
	}
	ruleGroupResponseCandidates := []string{"ACTIVE"}
	if !ackutil.InStrings(*r.ko.Status.RuleGroupResponse.RuleGroupStatus, ruleGroupResponseCandidates) {
		return false, nil
	}
