...
func (rm *resourceManager) IsSynced(ctx context.Context, res acktypes.AWSResource) (bool, error) {
	...
	if r.ko.Status.FirewallStatus == nil {
		return false, nil
	}
	if r.ko.Status.FirewallStatus.Status == nil {
		return false, nil
	}
	firewallStatusCandidates := []string{"READY"}
	if !ackutil.InStrings(*r.ko.Status.FirewallStatus.Status, firewallStatusCandidates) {
		return false, nil
	}
